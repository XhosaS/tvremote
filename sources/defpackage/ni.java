...
public final class ni {
    ...
    public ni(float f, float f2, float f3, float f4, float f5, float f6) {
        this.a = f;
        this.b = f2;
        this.c = f3;
        this.d = f4;
    ...
    static ni b(int i) {
        nr nrVar = nr.a;
        int i2 = nj.e;
        float fA = nj.a(Color.red(i));
        float fA2 = nj.a(Color.green(i));
        float fA3 = nj.a(Color.blue(i));
        float[][] fArr = nj.d;
        float[] fArr2 = fArr[0];
        ...
        float f21 = ((((40.0f * fSignum) + f19) + fSignum3) / 20.0f) * nrVar.c;
        float f22 = nrVar.b;
        float f23 = nrVar.e;
        float fPow4 = ((float) Math.pow(f21 / f22, nrVar.j * 0.69f)) * 100.0f;
        float fSqrt = (float) Math.sqrt(fPow4 / 100.0f);
        float f24 = nrVar.i;
        float f25 = fSqrt * 5.7971015f * (f22 + 4.0f) * f24;
        float fSqrt2 = ((float) Math.sqrt(fPow4 / 100.0d)) * ((float) Math.pow(1.64d - Math.pow(0.29d, nrVar.f), 0.73d)) * ((float) Math.pow(((((((float) (Math.cos((((((double) fAtan2) < 20.14d ? fAtan2 + 360.0f : fAtan2) * 3.141592653589793d) / 180.0d) + 2.0d) + 3.8d)) * 0.25f) * 3846.1538f) * nrVar.d) * ((float) Math.sqrt((r3 * r3) + (r5 * r5)))) / (((((fSignum * 20.0f) + f19) + (fSignum3 * 21.0f)) / 20.0f) + 0.305f), 0.9d));
        ...
        return new ni(fAtan2, fSqrt2, fArr11[0], fArr11[4], fArr11[5], f26);
    }

    public static ni c(float f, float f2, float f3) {
        nr nrVar = nr.a;
        float f4 = nrVar.e;
        Math.sqrt(f / 100.0d);
        float f5 = nrVar.b + 4.0f;
        float f6 = nrVar.i * f2;
        Math.sqrt(((f2 / ((float) Math.sqrt(r1))) * 0.69f) / f5);
        double d = (3.1415927f * f3) / 180.0f;
        float fLog = ((float) Math.log((f6 * 0.0228d) + 1.0d)) * 43.85965f;
        return new ni(f3, f2, f, (1.7f * f) / ((0.007f * f) + 1.0f), fLog * ((float) Math.cos(d)), fLog * ((float) Math.sin(d)));
    }

    final int a(nr nrVar) {
        float f = this.b;
        float fSqrt = 0.0f;
        if (f != 0.0d) {
            double d = this.c;
        ...
        float fPow = (float) Math.pow(fSqrt / Math.pow(1.64d - Math.pow(0.29d, nrVar.f), 0.73d), 1.1111111111111112d);
        double d2 = (this.a * 3.1415927f) / 180.0f;
        double dCos = Math.cos(2.0d + d2) + 3.8d;
        float f2 = nrVar.e;
        float fPow2 = nrVar.b * ((float) Math.pow(this.c / 100.0d, 1.4492753673265821d / nrVar.j));
        float f3 = ((float) dCos) * 0.25f * 3846.1538f * nrVar.d;
        float f4 = fPow2 / nrVar.c;
        float fSin = (float) Math.sin(d2);
        float fCos = (float) Math.cos(d2);
        ...
        float[] fArr5 = fArr2[2];
        return ns.b(f14 + f15 + f16, f17 + f18 + f19, (f11 * fArr5[0]) + (f12 * fArr5[1]) + (f13 * fArr5[2]));
    }
}
