...
public abstract class CronetProvider {
    ...
    private static boolean addCronetProviderFromResourceFile(Context context, zzv zzvVar, Set<zyh> set) {
        ...
        if (string != null && !string.equals(PLAY_SERVICES_CRONET_PROVIDER_CLASS) && !string.equals(GMS_CORE_CRONET_PROVIDER_CLASS) && !string.equals(JAVA_CRONET_PROVIDER_CLASS) && !string.equals(NATIVE_CRONET_PROVIDER_CLASS)) {
            z = true;
            if (!addCronetProviderImplByClassName(context, string, zzvVar, set, true)) {
                Log.e(TAG, a.cg(string, "Unable to instantiate Cronet implementation class ", " that is listed as in the app string resource file under CronetProviderClassName key"));
            }
        }
        return z;
    }

    private static boolean addCronetProviderImplByClassName(Context context, String str, zzv zzvVar, Set<zyh> set, boolean z) throws NoSuchMethodException, SecurityException {
        try {
            ...
            zyh zyhVar = new zyh();
            zyhVar.a = (CronetProvider) constructor.newInstance(context);
            zyhVar.b = zzvVar;
            set.add(zyhVar);
            return true;
    ...
    static List<zyh> getAllProviderInfos(Context context) throws NoSuchMethodException, SecurityException {
        LinkedHashSet linkedHashSet = new LinkedHashSet();
        addCronetProviderFromResourceFile(context, zzv.CRONET_SOURCE_UNSPECIFIED, linkedHashSet);
        zzv zzvVar = zzv.CRONET_SOURCE_PLAY_SERVICES;
        addCronetProviderImplByClassName(context, PLAY_SERVICES_CRONET_PROVIDER_CLASS, zzvVar, linkedHashSet, false);
        addCronetProviderImplByClassName(context, GMS_CORE_CRONET_PROVIDER_CLASS, zzvVar, linkedHashSet, false);
        addCronetProviderImplByClassName(context, NATIVE_CRONET_PROVIDER_CLASS, zzv.CRONET_SOURCE_STATICALLY_LINKED, linkedHashSet, false);
        addCronetProviderImplByClassName(context, JAVA_CRONET_PROVIDER_CLASS, zzv.CRONET_SOURCE_FALLBACK, linkedHashSet, false);
    ...
    public static List<CronetProvider> getAllProviders(Context context) {
        ...
        Iterator<zyh> it = getAllProviderInfos(context).iterator();
        while (it.hasNext()) {
