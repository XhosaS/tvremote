...
public abstract class CronetProvider {
    ...
    private static boolean addCronetProviderFromResourceFile(Context context, aiad aiadVar, Set set) {
        ...
        if (string != null && !string.equals(PLAY_SERVICES_CRONET_PROVIDER_CLASS) && !string.equals(GMS_CORE_CRONET_PROVIDER_CLASS) && !string.equals(JAVA_CRONET_PROVIDER_CLASS) && !string.equals(NATIVE_CRONET_PROVIDER_CLASS)) {
            z = true;
            if (!addCronetProviderImplByClassName(context, string, aiadVar, set, true)) {
                Log.e(TAG, a.e(string, "Unable to instantiate Cronet implementation class ", " that is listed as in the app string resource file under CronetProviderClassName key"));
            }
        }
        return z;
    }

    private static boolean addCronetProviderImplByClassName(Context context, String str, aiad aiadVar, Set set, boolean z) throws NoSuchMethodException, SecurityException {
        try {
            ...
            ahycVar.a = (CronetProvider) constructor.newInstance(context);
            ahycVar.b = aiadVar;
            set.add(ahycVar);
            return true;
    ...
    static List getAllProviderInfos(Context context) throws NoSuchMethodException, SecurityException {
        ...
        addCronetProviderFromResourceFile(context, aiad.CRONET_SOURCE_UNSPECIFIED, linkedHashSet);
        aiad aiadVar = aiad.CRONET_SOURCE_PLAY_SERVICES;
        addCronetProviderImplByClassName(context, PLAY_SERVICES_CRONET_PROVIDER_CLASS, aiadVar, linkedHashSet, false);
        addCronetProviderImplByClassName(context, GMS_CORE_CRONET_PROVIDER_CLASS, aiadVar, linkedHashSet, false);
        addCronetProviderImplByClassName(context, NATIVE_CRONET_PROVIDER_CLASS, aiad.CRONET_SOURCE_STATICALLY_LINKED, linkedHashSet, false);
        addCronetProviderImplByClassName(context, JAVA_CRONET_PROVIDER_CLASS, aiad.CRONET_SOURCE_FALLBACK, linkedHashSet, false);
    ...
    private static void logReflectiveOperationException(String str, boolean z, Exception exc) {
        if (z) {
            ...
            if (Log.isLoggable(str2, 3)) {
                Log.d(str2, a.e(str, "Tried to load ", " provider class but it wasn't included in the app classpath"));
            }
        }
    }
