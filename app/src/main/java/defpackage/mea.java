...
final class mea {
    ...
    public static Object a(Future future, long j, TimeUnit timeUnit) throws ExecutionException, InterruptedException, TimeoutException {
        if (!a || (future instanceof zum) || (future instanceof zye) || (future instanceof mdz) || TimeUnit.NANOSECONDS.convert(j, timeUnit) <= 2147483647999999999L || future.isDone()) {
    ...
    public static RunnableScheduledFuture b(RunnableScheduledFuture runnableScheduledFuture) {
        return a ? new mdz(runnableScheduledFuture) : runnableScheduledFuture;
    }
}
